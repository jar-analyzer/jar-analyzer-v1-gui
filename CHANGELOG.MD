...
重要更新版本，变化较大建议下载更新，整体重构并新增多个功能

重构UI为中文版黑暗主题；重写代码面板实现类似IDEA的效果；增加文件树面板实现类似JD-GUI的功能；优化定位算法；增加搜索字符串常量的功能，分析`LDC`指令实现；增加无脑搜索，分析某些指令实现任何地方遇到包含字符串都会精确输出对应的方法的功能；增加两个菜单

从该版本开始，仅支持`JDK11`以上，因为某些组件不能在`JDK8`正常运行，不过会提供包含`JRE`的`EXE`版本
